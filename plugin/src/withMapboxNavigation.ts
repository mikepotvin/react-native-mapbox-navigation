...
let pkg: { name: string; version?: string } = {
  name: "@everdriven/react-native-mapbox-navigation",
};
try {
  pkg = require("@everdriven/react-native-mapbox-navigation/package.json");
} catch {
  // empty catch block
}
...
export function addConstantBlock(
  ...
  const tag = `@everdriven/react-native-mapbox-navigation-rbmbnaversion`;
...
export function addDisableOutputPathsBlock(src: string): string {
  const tag = `@everdriven/react-native-mapbox-navigation-rbmbnatop`;
...
export function addMapboxInstallerBlock(
  ...
  return mergeContents({
    tag: `@everdriven/react-native-mapbox-navigation-${blockName}_installer`,
    src,
...
const addLibCppFilter = (appBuildGradle: string): string => {
  ...
  return mergeContents({
    tag: `@everdriven/react-native-mapbox-navigation-libcpp`,
    src: appBuildGradle,
...
const addMapboxMavenRepo = (projectBuildGradle: string): string => {
  ...
  return mergeContents({
    tag: `@everdriven/react-native-mapbox-navigation-v2-maven`,
